    ...
    describe '#create and #save' do
      ...
      describe '#new' do
        context 'when an assignment requires quiz' do
          ...
          it 'shows error message and redirects to submitted_folder#view if current participant does not have a team' do
            ...
            expect(response).to redirect_to('/submitted_folder/view?id=1')
          end

          it 'shows error message and redirects to submitted_folder#view if current participant have a team w/o topic' do
            team = double('AssignmentTeam')
            allow(AssignmentParticipant).to receive_message_chain(:find, :team).with('1').with(no_args).and_return(team)
            allow(@assignment).to receive(:topics?).and_return(true)
        ...
        context 'when an assignment does not require quiz' do
          it 'shows error message and redirects to submitted_folder#view' do
            request_params = { aid: 1,
                       model: 'QuizQuestionnaire',
                       pid: 1,
                       private: 0 }
      ...
      describe '#edit' do
        ...
        context 'when current questionnaire has been taken by someone' do
          it 'shows flash[:error] message and redirects to submitted_folder#view page' do
            stub_current_user(student, student.role.name, student.role) # action only permitted for Student role
      ...
      describe '#update' do
        context 'when @questionnaire is nil' do
          it 'redirects to submitted_folder#view page' do
        ...
        context 'when @questionnaire is not nil' do
          it 'updates all quiz questions and redirects to submitted_folder#view page' do
            request_params = { id: 1,
                       pid: 1,
                       save: true,
                       questionnaire: { name: 'test questionnaire',
